...
$(function(){
    ...
    if(project_id.length == 0){
        project_id = edit_data[0].project_id
    }
	...
	let a = $(document).ready(function(){
        ...
        mapping_data_img();
        setTimeout(() => {
            var node1 = treeObj.getNodeByTId("treeDemo_1");
            var node2 = treeObj.getNodeByTId("treeDemo_2");
            var node = treeObj.getNodeByTId("treeDemo_3");
            // console.log("CHECK",node);
            treeObj.selectNode(node);
            treeObj.expandNode(node2, true, true, true);
            // var project_open = node1.open;
            // var location_open = node2.open;
    ...
    function getnodeid(treeId, treeNode, clickFlag) {
        ...
        setTimeout(() => {
            ...
            if (selecttype == 'p'){
                ...
                treenode_parent = treeNode.getParentNode()
                var treeObj = $.fn.zTree.getZTreeObj(treeId);
                treeObj.expandNode(treenode_parent, true, true, true);
        ...
        $("#maptype input[type='radio']").each(function() {
            ...
            if(checked){
                if($(this).val() == "google"){
                    ...
                    kakao_map.relayout();
                    kakao_map.setCenter(new kakao.maps.LatLng(mapinfodata[map_idx]["lat"],  mapinfodata[map_idx]["lng"]));
...
function GoogleMap() {
    ...
    google_map = new google.maps.Map(document.getElementById("google_map_wrap"), {
        zoom: 14,                                                               // 높을수록 확대
        center: { lat: mapinfodata[map_idx]["lat"], lng: mapinfodata[map_idx]["lng"] },
    });
...
function KakaoMap() {
    ...
    var options = {                                                 //지도를 생성할 때 필요한 기본 옵션
        center: new kakao.maps.LatLng( mapinfodata[map_idx]["lat"],  mapinfodata[map_idx]["lng"]),       //지도의 중심좌표.
...
function getmapinfo(){
    ...
    $.ajax({
        type: "GET",
        url: "/station/mapinfo?project_id="+project_id,
        async : false,
        success : function(json) {
            edit_data = json.data;
            console.log(edit_data)
            ...
            for(var i = 0; i < edit_data.length; i++){
                if(station_list.indexOf(edit_data[i].station_id) == -1){
                    ...
                    if(edit_data[i].station_id == station_id){
                        console.log(mapinfodata)
                        map_idx = mapinfodata.length-1
                        console.log("map_idx", map_idx)
                    }
                }
