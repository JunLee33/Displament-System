...
class stationModel(db.Model):
    ...
    @classmethod
    def monitoring_lo_st_station_id(cls, project_id):
        sql  = """select  e.station_id from tbl_station e 
                inner join (select c.station_seq, d.location_id, d.location_name, d.project_id from  tbl_location_station c left join (
                select a.location_id, a.location_name, b.project_id from tbl_location a 
                left join tbl_project b on a.project_id = b.project_id and a.use_yn = 'Y' and b.use_yn = 'Y') d on c.location_id = d.location_id
                where c.use_yn = 'Y') f on e.station_seq = f.station_seq
